...
test.describe('Map App - Page Load', () => {
  ...
  test('page loads with all UI elements', async ({ page }) => {
    await expect(page).toHaveTitle(/地圖應用程式/);
    await expect(page.locator('h1')).toHaveText('地圖應用程式');
    await expect(page.locator('p')).toHaveText('按港鐵站搜尋');
  ...
  test('search input has Chinese placeholder', async ({ page }) => {
    const placeholder = await page.locator('#searchInput').getAttribute('placeholder');
    expect(placeholder).toContain('輸入地點或地址');
  });
});
...
test.describe('Map App - Dropdowns', () => {
  ...
  test('Hong Kong Island populates lines in Chinese', async ({ page }) => {
  ...
  test('Kowloon lines in Chinese', async ({ page }) => {
